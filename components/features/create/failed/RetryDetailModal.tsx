...
export function RetryDetailModal({ post, onClose, onConfirmReschedule, onEdit }: RetryDetailModalProps) {
  ...
  const handleConfirmClick = () => {
    if (!rescheduleDate || !rescheduleTime) {
        toast.error(tToast('invalidDateTime'));
        return;
    }
    ...
    if (targetDateTime.getTime() < now.getTime()) {
        toast.error(tToast('pastScheduleError'));
        return;
    }
  ...
  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={onClose}>
      <div className="bg-[#1E1E23] border border-white/10 rounded-2xl w-[600px] max-w-[95vw] shadow-xl max-h-[90vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="px-6 pt-6 pb-4 border-b border-white/10"><div className="flex items-center justify-between"><h3 className="text-xl font-bold text-white">{t('title')}</h3><button onClick={onClose} className="text-white/60 hover:text-white"><CloseIcon className="w-6 h-6" /></button></div></div>
