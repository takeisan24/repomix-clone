  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-4 mb-2 mt-1 mr-8">
      ...
      <div className="inline-flex rounded-lg overflow-hidden border border-white/10 ml-auto md:ml-4">
        <Button variant={calendarView === 'monthly' ? 'secondary' : 'ghost'} size="sm" onClick={() => onSetView('monthly')} className={calendarView === 'monthly' ? 'bg-white/10' : ''}>{t('month')}</Button>
        <Button variant={calendarView === 'weekly' ? 'secondary' : 'ghost'} size="sm" onClick={() => onSetView('weekly')} className={calendarView === 'weekly' ? 'bg-white/10' : ''}>{t('week')}</Button>
      </div>
    </div>
  );
}